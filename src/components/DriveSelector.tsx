...
export default function DriveSelector() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 h-7 px-2.5 rounded text-xs font-medium bg-stone-100 text-stone-700 hover:bg-stone-200 transition-colors max-w-[200px]"
      ...
      {open && (
        <div
          role="listbox"
          className="absolute top-full left-0 mt-1 w-64 rounded-md border border-stone-200 bg-white shadow-lg z-50 text-xs flex flex-col"
          style={{ maxHeight: "320px" }}
        >
          {/* Search input — sticky at top */}
          <div className="px-2 pt-2 pb-1 border-b border-stone-100 shrink-0">
            <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-stone-100">
              <svg className="w-3 h-3 text-stone-400 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
              ...
              <input
                ...
                className="flex-1 bg-transparent outline-none text-xs text-stone-700 placeholder:text-stone-400"
              />
              {query && (
          ...
          <div className="overflow-y-auto flex-1 py-1">
            {!hasResults && (
              <div className="px-3 py-3 text-stone-400 text-center">No results</div>
            )}
  ...
  return (
    <button
      ...
      className={[
        ...
        active ? "bg-stone-100 text-stone-900 font-medium" : "text-stone-700 hover:bg-stone-50",
      ].join(" ")}
    >
      {icon}
      <span className="truncate">{label}</span>
      {active && (
        <svg className="ml-auto w-3 h-3 shrink-0 text-stone-500" viewBox="0 0 12 12">
...
function DriveIcon() {
  return (
    <svg className="w-3.5 h-3.5 shrink-0 text-stone-500" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
