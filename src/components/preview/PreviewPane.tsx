...
function DragHandle({ onDelta }: { onDelta: (d: number) => void }) {
  ...
  return (
    <div
      ...
      className="h-2 shrink-0 flex items-center justify-center cursor-row-resize group select-none bg-stone-50 border-y border-stone-100 hover:bg-blue-50 hover:border-blue-200 transition-colors"
    >
      <div className="w-8 h-0.5 rounded-full bg-stone-300 group-hover:bg-blue-400 transition-colors" />
    </div>
  );
}
...
function SectionHeader({ title, count, collapsed, onToggle }: {
  ...
  return (
    <button
      onClick={onToggle}
      className="flex items-center gap-1.5 px-3 py-1.5 w-full text-left bg-stone-50 border-b border-stone-100 shrink-0 hover:bg-stone-100 transition-colors"
    >
      <svg
        className={`w-3 h-3 text-stone-400 shrink-0 transition-transform ${collapsed ? "-rotate-90" : ""}`}
        viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"
      >
        <polyline points="2,4 6,8 10,4" />
      </svg>
      <span className="text-[10px] font-semibold uppercase tracking-wider text-stone-500">{title}</span>
      <span className="ml-auto text-[10px] tabular-nums text-stone-400">{count}</span>
    </button>
  );
}
        ...
        {selectedFile ? (
          <>
            ...
            <div className="px-3 py-2 border-b border-stone-100 shrink-0">
              <p className="font-medium text-stone-800 truncate text-xs">{selectedFile.name}</p>
              <p className="text-[10px] text-stone-400 mt-0.5 truncate">{selectedFile.mimeType}</p>
            </div>
            ...
            <div className={`flex-1 min-h-0 ${isDoc ? "overflow-hidden" : "overflow-auto"}`}>
              ...
              {selectedKind === "other" && (
                <p className="p-4 text-stone-400 text-xs">No preview for this file type.</p>
              )}
              {selectedKind === "folder" && (
                <p className="p-4 text-stone-400 text-xs">Folders don't have a preview.</p>
              )}
            </div>
          </>
        ) : (
